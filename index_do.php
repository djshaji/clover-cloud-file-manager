...
$doSecret = json_decode (file_get_contents ("../do-secrets/spaces.json"), true);
$spaces = new Spaces($doSecret ["key"], $doSecret ["secret"], $doSecret ["region"]);
...
$space = $spaces->space('alexis');
$parent = $_GET ["dir"] ;
$files = null ;
$json = array ();
if ($parent)
  $files = $space->listFiles($parent);
else 
...
foreach ($files['files'] as $file) {
  ...
  if ($p == null)
    $p = "/";
  if (strpos ($file->filename, $p) === 0) {
    $vector = explode ("/", $file->filename);
    if (sizeof ($vector) > 2)
      $json [$vector [1]] = null;  
    else
      $json [$vector [1]] = $file->getUrl () ;
  }
  else 
    $json [explode ("/", $file->filename) [0]] = null;
}
...
// var_dump ($cmd) ;
?><div class="alert bg-warning text-white"><?php
var_dump ($json);?></div><?php
// var_dump ($output);
// var_dump ($res);
?>
<script>
        ...
        if ($url == null) { // folder
          ...
          ?>
            <div class="col-md-1 justify-content-center text-center">
              <a href="<?php echo "?dir=$dirname" ;?>"> <img src="folder.png" width=64></img>
